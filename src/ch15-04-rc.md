...
<img alt="Una lista enlazada con la etiqueta 'a' que apunta a tres elementos: el primer elemento contiene el entero 5 y apunta al segundo elemento. El segundo elemento contiene el entero 10 y apunta al tercer elemento. El tercer elemento contiene el valor 'Nil', que indica el final de la lista; no apunta a ningún lado. Una lista enlazada con la etiqueta 'b' apunta a un elemento que contiene el entero 3 y apunta al primer elemento de la lista 'a'. Una lista enlazada con la etiqueta 'c' apunta a un elemento que contiene el entero 4 y también apunta al primer elemento de la lista 'a', de modo que la cola de las listas 'b' y 'c' es la misma lista 'a'" src="img/trpl15-03.svg" class="center" />
...
está en el [prelude][prelude]. En `main`, creamos la lista que contiene `5` y 
`10` y la almacenamos en un nuevo `Rc<List>` en `a`. Luego cuando creamos `b` y 
`c`, llamamos a la función `Rc::clone` y pasamos una referencia al `Rc<List>` en 
`a` como argumento.
